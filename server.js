...
function touchSession(token) {
  ...
  if (s) { s.lastActive = now(); touchUser(s.username); }
}
...
function publicUser(u) { return { username: u.username, friends: Array.from(u.friends).map(friendPublic).filter(Boolean), avatar: u.avatar || null, bio: u.bio || '', }; }

function sweep() {
  const cutoff = now() - INACTIVITY_LIMIT_MS;
  for (const [token, s] of sessions) { if (s.lastActive < cutoff) sessions.delete(token); }
  for (const [username, u] of users) { if (u.lastActive < cutoff) { users.delete(username); callSignals.delete(username); typingStatus.delete(username); for (const other of users.values()) { other.friends.delete(username); } } }
  messages = messages.filter((m) => { if (m.ts < cutoff) return false; if (!users.has(m.from) || !users.has(m.to)) return false; return true; });
  ...
  for (const [from, info] of typingStatus) { if (info.ts < typingCutoff || !users.has(from) || !users.has(info.to)) { typingStatus.delete(from); } }
}
...
app.post('/create', async (req, res) => {
  ...
  users.set(cleanUsername, { username: cleanUsername, passwordHash, lastActive: now(), friends: new Set(), avatar: null, bio: '', idle: false, });
...
app.post('/heartbeat', requireAuth, (req, res) => {
  ...
  if (u) u.idle = !!(req.body && req.body.idle);
...
app.post('/profile', requireAuth, (req, res) => {
  ...
  if (avatar !== undefined) { if (avatar === null || avatar === '') { u.avatar = null; } else if (typeof avatar === 'string' && /^data:image\/(png|jpeg|jpg|webp|gif);base64,/.test(avatar)) { if (avatar.length > MAX_AVATAR_LEN) { return res.status(400).json({ error: 'Avatar image is too large.' }); } u.avatar = avatar; } else { return res.status(400).json({ error: 'Invalid avatar format.' }); } }
  if (bio !== undefined) { if (typeof bio !== 'string' || bio.length > MAX_BIO_LEN) { return res.status(400).json({ error: 'Bio must be a string up to ' + MAX_BIO_LEN + ' characters.' }); } u.bio = bio.trim(); }
...
app.post('/typing', requireAuth, (req, res) => {
  ...
  if (!me || !me.friends.has(to)) { return res.status(403).json({ error: 'You must both be added as friends.' }); }
...
app.post('/addfriend', requireAuth, (req, res) => {
  ...
  if (!users.has(target)) { return res.status(404).json({ error: 'No such user (they may be offline/expired).' }); }
  const me = users.get(req.username);
  const them = users.get(target);
  me.friends.add(target);
  them.friends.add(req.username);
  res.json({ ok: true, friends: Array.from(me.friends).map(friendPublic).filter(Boolean) });
});

app.get('/friends', requireAuth, (req, res) => {
  const me = users.get(req.username);
  res.json({ friends: Array.from(me.friends).map(friendPublic).filter(Boolean) });
});
...
app.post('/send', requireAuth, (req, res) => {
  ...
  if (!users.has(to)) { return res.status(404).json({ error: 'Recipient not found (may be offline/expired).' }); }
  const me = users.get(req.username);
  if (!me.friends.has(to)) { return res.status(403).json({ error: 'You must both be added as friends to message.' }); }
  const msg = { id: crypto.randomUUID(), from: req.username, to, ciphertext, iv, ts: now(), delivered: false, };
  messages.push(msg);
...
app.get('/receive', requireAuth, (req, res) => {
  ...
  res.json({ messages: inbox.map((m) => ({ id: m.id, from: m.from, ciphertext: m.ciphertext, iv: m.iv, ts: m.ts, })), });
});
...
app.post('/call/signal', requireAuth, (req, res) => {
  ...
  if (!users.has(to)) { return res.status(404).json({ error: 'Recipient not found (may be offline/expired).' }); }
  const me = users.get(req.username);
  if (!me.friends.has(to)) { return res.status(403).json({ error: 'You must both be added as friends to call.' }); }
...
app.post('/request', requireAuth, (req, res) => {
  ...
  const me = users.get(req.username);
  const relevant = messages.filter((m) => (m.to === req.username || m.from === req.username) && m.ts > since);
  ...
  res.json({ serverTime: now(), friends: Array.from(me.friends).map(friendPublic).filter(Boolean), typing: typingFrom, messages: relevant.map((m) => ({ id: m.id, from: m.from, to: m.to, ciphertext: m.ciphertext, iv: m.iv, ts: m.ts, })), });
});

app.get('/history', requireAuth, (req, res) => {
  const relevant = messages.filter((m) => m.to === req.username || m.from === req.username);
  res.json({ serverTime: now(), messages: relevant.map((m) => ({ id: m.id, from: m.from, to: m.to, ciphertext: m.ciphertext, iv: m.iv, ts: m.ts, })), });
});

app.get('/', (req, res) => { res.json({ status: 'ok', users: users.size, messages: messages.length }); });
